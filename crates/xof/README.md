...
Minimal, dependency-free SHAKE-256 extendable-output function (XOF) built
directly on the Keccak-f[1600] permutation. Serves two roles in the Algorand
state proof stack: seeding Sumhash512 (the primary Merkle tree hash) and
driving the pseudorandom coin generation inside the verification protocol.
...
`Shake256` follows a strict absorb → flip → squeeze sequence:

```rust
use xof::Shake256;

let mut shake = Shake256::new();
shake.absorb(b"seed material"); // feed input; may be called multiple times
shake.flip();                   // finalise — no more absorb calls after this
let mut out = [0u8; 8];
