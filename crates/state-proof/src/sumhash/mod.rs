...
mod tests {
    ...
    fn update_incremental_matches_single() {
        ...
        let mut h_incremental = Sumhash512::new();
        h_incremental.update(&data[..50]);
        h_incremental.update(&data[50..100]);
        h_incremental.update(&data[100..170]);
        h_incremental.update(&data[170..]);

        assert_eq!(h_single.0.state, h_incremental.0.state);
        assert_eq!(h_single.0.pos,   h_incremental.0.pos);
        assert_eq!(h_single.0.total_len, h_incremental.0.total_len);
    }
