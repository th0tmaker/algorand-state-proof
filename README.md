...
Each selected signature is bundled with a cryptographic `Merkle` membership proof that ties it back to a known list of eligible signers — 
so anyone can independently confirm the signatures are legitimate without having to trust whoever assembled the proof.

The State Proof message carries forward a **participant commitment** and **ln(ProvenWeight)** for the next interval — together forming a `TrustAnchor` — creating a verifiable chain where every validated proof yields the anchor needed to verify the one that follows. A single trusted starting point (e.g. genesis commitment) is sufficient to validate an entire unbroken sequence of proofs.
